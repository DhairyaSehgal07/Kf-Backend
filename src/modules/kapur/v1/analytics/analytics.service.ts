...
export interface StorageLocationWiseFilters {
  dateFrom?: string;
  dateTo?: string;
  variety?: string;
  chamber?: string;
...
export interface StorageLocationQuantityRow {
  initialQuantity: number;
  currentQuantity: number;
  quantityRemoved: number;
}

export interface StorageLocationVarietyBreakdown extends StorageLocationQuantityRow {
  variety: string;
}

export interface StorageLocationRowSummary extends StorageLocationQuantityRow {
  row: string;
  varietyCount: number;
  varieties: StorageLocationVarietyBreakdown[];
}

export interface StorageLocationFloorSummary extends StorageLocationQuantityRow {
  floor: string;
  varietyCount: number;
  varieties: StorageLocationVarietyBreakdown[];
  rows: StorageLocationRowSummary[];
}

export interface StorageLocationChamberSummary extends StorageLocationQuantityRow {
  chamber: string;
  varietyCount: number;
  varieties: StorageLocationVarietyBreakdown[];
  floors: StorageLocationFloorSummary[];
}

export interface StorageLocationWiseResult {
  chambers: StorageLocationChamberSummary[];
  totals: StorageLocationQuantityRow & {
    varietyCount: number;
    varieties: StorageLocationVarietyBreakdown[];
  };
}

type QuantityAccumulator = { initial: number; current: number };

interface LocationNode {
  totals: QuantityAccumulator;
  varieties: Map<string, QuantityAccumulator>;
}

function accumulateQuantity(
  map: Map<string, QuantityAccumulator>,
  key: string,
  initial: number,
  current: number
): void {
...
function addToNode(
  ...
  node.totals.initial += initial;
  node.totals.current += current;
  accumulateQuantity(node.varieties, variety, initial, current);
}

function varietiesToArray(
  varieties: Map<string, QuantityAccumulator>
): StorageLocationVarietyBreakdown[] {
  return Array.from(varieties.entries())
    .map(([variety, { initial, current }]) => ({
      variety,
      ...toQuantityRow(initial, current),
    }))
    .sort((a, b) => a.variety.localeCompare(b.variety));
}

function createLocationNode(): LocationNode {
  return { totals: { initial: 0, current: 0 }, varieties: new Map() };
}
...
/**
 * Location-wise storage analytics from storage gate passes.
 * Returns a hierarchical breakdown: chamber → floor → row, with variety summaries at each level.
 * Optional dateFrom/dateTo filter by gate pass date; optional chamber/floor/row/variety filters.
 */
export async function getStorageLocationWiseAnalytics(
  coldStorageId: string,
  filters: StorageLocationWiseFilters,
  logger?: FastifyBaseLogger
): Promise<StorageLocationWiseResult> {
  const emptyTotals = {
    ...toQuantityRow(0, 0),
    varietyCount: 0,
    varieties: [] as StorageLocationVarietyBreakdown[],
  };
  ...
  if (farmerStorageLinkIds.length === 0) {
    return { chambers: [], totals: emptyTotals };
  }
  ...
  const chamberFilter =
    filters.chamber != null && filters.chamber.trim() !== ''
      ? filters.chamber.trim()
      : undefined;
  const floorFilter =
    filters.floor != null && filters.floor.trim() !== ''
      ? filters.floor.trim()
      : undefined;
  const rowFilter =
    filters.row != null && filters.row.trim() !== ''
      ? filters.row.trim()
      : undefined;
  ...
  type RowNode = LocationNode;
  type FloorNode = LocationNode & { rows: Map<string, RowNode> };
  type ChamberNode = LocationNode & { floors: Map<string, FloorNode> };

  const chamberMap = new Map<string, ChamberNode>();
  const overallTotals = createLocationNode();

  for (const doc of docs) {
    const variety = doc.variety?.trim() || 'Unspecified';
    for (const bs of doc.bagSizes ?? []) {
      const chamber = bs.chamber?.trim() || '';
      const floor = bs.floor?.trim() || '';
      const row = bs.row?.trim() || '';

      if (chamberFilter && chamber !== chamberFilter) continue;
      if (floorFilter && floor !== floorFilter) continue;
      if (rowFilter && row !== rowFilter) continue;

      const initial = Number(bs.initialQuantity) || 0;
      const current = Number(bs.currentQuantity) || 0;

      let chamberNode = chamberMap.get(chamber);
      if (!chamberNode) {
        chamberNode = { ...createLocationNode(), floors: new Map() };
        chamberMap.set(chamber, chamberNode);
      }
      let floorNode = chamberNode.floors.get(floor);
      if (!floorNode) {
        floorNode = { ...createLocationNode(), rows: new Map() };
        chamberNode.floors.set(floor, floorNode);
      }
      let rowNode = floorNode.rows.get(row);
      if (!rowNode) {
        rowNode = createLocationNode();
        floorNode.rows.set(row, rowNode);
      }

      addToNode(chamberNode, variety, initial, current);
      addToNode(floorNode, variety, initial, current);
      addToNode(rowNode, variety, initial, current);
      addToNode(overallTotals, variety, initial, current);
    }
  }

  const chambers: StorageLocationChamberSummary[] = Array.from(
    chamberMap.entries()
  )
    .map(([chamber, chamberNode]) => {
      const floors: StorageLocationFloorSummary[] = Array.from(
        chamberNode.floors.entries()
      )
        .map(([floor, floorNode]) => {
          const rows: StorageLocationRowSummary[] = Array.from(
            floorNode.rows.entries()
          )
            .map(([row, rowNode]) => {
              const varieties = varietiesToArray(rowNode.varieties);
              return {
                row,
                ...toQuantityRow(
                  rowNode.totals.initial,
                  rowNode.totals.current
                ),
                varietyCount: varieties.length,
                varieties,
              };
            })
            .sort((a, b) => compareLocationNames(a.row, b.row));

          const floorVarieties = varietiesToArray(floorNode.varieties);
          return {
            floor,
            ...toQuantityRow(
              floorNode.totals.initial,
              floorNode.totals.current
            ),
            varietyCount: floorVarieties.length,
            varieties: floorVarieties,
            rows,
          };
        })
        .sort((a, b) => compareLocationNames(a.floor, b.floor));

      const chamberVarieties = varietiesToArray(chamberNode.varieties);
      return {
        chamber,
        ...toQuantityRow(
          chamberNode.totals.initial,
          chamberNode.totals.current
        ),
        varietyCount: chamberVarieties.length,
        varieties: chamberVarieties,
        floors,
      };
    })
    .sort((a, b) => compareLocationNames(a.chamber, b.chamber));

  const overallVarieties = varietiesToArray(overallTotals.varieties);
  const totals = {
    ...toQuantityRow(
      overallTotals.totals.initial,
      overallTotals.totals.current
    ),
    varietyCount: overallVarieties.length,
    varieties: overallVarieties,
  };
  ...
  logger?.info(
    ...
    'Storage location-wise analytics computed'
  );

  return { chambers, totals };
}
